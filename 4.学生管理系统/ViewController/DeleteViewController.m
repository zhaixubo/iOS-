...
- (void)pressok {
    if ([_firstTextField.text isEqualToString:@""]) {
        ...
        if (boo == 0) {
            ...
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"通知" message:@"删除成功" preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *sure = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                [_delegate newdelete:_namearray :_classarray :_gradearray];
                [self dismissViewControllerAnimated:YES completion:nil];
            }];
            [alertController addAction:sure];
            [self presentViewController:alertController animated:YES completion:nil];
            ...
            if ([_secondTextField.text isEqualToString:@""]) {
                ...
                if (boo == 20) {
                    ...
                    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"通知" message:@"删除成功" preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction *sure = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                        [_namearray removeObjectAtIndex:boo2];
                        [_classarray removeObjectAtIndex:boo2];
                        [_gradearray removeObjectAtIndex:boo2];
                        [_delegate newdelete:_namearray :_classarray :_gradearray];
                        [self dismissViewControllerAnimated:YES completion:nil];
                    }];
                    [alertController addAction:sure];
