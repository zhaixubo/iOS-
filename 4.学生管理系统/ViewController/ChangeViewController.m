...
- (void)viewDidLoad {
    ...
    _tishilabel.text = @"请先查找您要修改的信息";
    _tishilabel.frame = CGRectMake(40, 350, 300, 30);
    ...
    if(_rightbutton.tag == 1) {
        if ([_firstTextField.text isEqualToString:@""]) {
            ...
            if (boo == 0) {
                ...
                _alertController = [UIAlertController alertControllerWithTitle:@"通知" message:@"查找成功" preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction *sure = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    [self.view addSubview:_thirdTextField];
                    _tishilabel.text = @"请输入您修改后的信息";
...
- (void)xiugai {
    if ([_firstTextField.text isEqualToString: @"" ]|| [_secondTextField.text isEqualToString: @"" ] || [_thirdTextField.text isEqualToString: @"" ]) {
        ...
        int boo = 0;
        for (int i = 0; i < _namearray.count; i++) {
            if ([_firstTextField.text isEqualToString:_namearray[i]] && [_secondTextField.text isEqualToString:_classarray[i]] && i != t) {
                boo = 1;
                break;
            }
        }
        if (boo == 1) {
            ...
            if (([firstString isEqualToString:@"1"] || [firstString isEqualToString:@"2"] || [firstString isEqualToString:@"3"] || [firstString isEqualToString:@"4"] || [firstString isEqualToString:@"5"]) && [secondString isEqualToString:@"班"] && (grade >= 0 || grade <= 100)&& _secondTextField.text.length == 2) {
                ...
                UIAlertAction *sure = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    ...
                    [_namearray replaceObjectAtIndex:t withObject:_firstTextField.text];
                    [_classarray replaceObjectAtIndex:t withObject:_secondTextField.text];
                    [_gradearray replaceObjectAtIndex:t withObject:_thirdTextField.text];
                    [_delegate newchange:_namearray :_classarray :_gradearray];
                    [self dismissViewControllerAnimated:YES completion:nil];
                }];
                [_alertController addAction:sure];
                [self presentViewController:_alertController animated:YES completion:nil];
